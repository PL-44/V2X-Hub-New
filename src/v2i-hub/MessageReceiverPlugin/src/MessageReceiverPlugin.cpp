...
namespace MessageReceiver
{
	...
	BsmMessage *MessageReceiverPlugin::DecodeBsmNew(uint32_t vehicleId, uint32_t heading, uint32_t speed, uint32_t latitude,
													uint32_t longitude, uint32_t elevation)
	{
		PLOG(logERROR) << "BSM vehicleId: " << vehicleId
					   << ", heading: " << heading
					   << ", speed: " << speed
					   << ", latitude: " << latitude
					   << ", longitude: " << longitude
		...
		return new tmx::messages::BsmMessage(message);
	}
												 ...
												 uint32_t longitude, uint32_t elevation, DecodedBsmMessage &decodedBsm)
	{
		...
		PLOG(logERROR) << " Decoded BSM: " << decodedBsm;
	...
	void MessageReceiverPlugin::OnMessageReceived(routeable_message &msg)
	{
		...
		MessageFrameMessage frame_msg;

		int msgPSID = api::msgPSID::None_PSID;

		PLOG(logERROR) << "Received a Message DEBUG, type: " << msg.get_type() << " subtype: " << msg.get_subtype();

		if (msg.get_type() == "Unknown" && msg.get_subtype() == "Unknown")
		{
			PLOG(logERROR) << "Message type and subtype is unknown";
			if (msg.get_encoding() == api::ENCODING_JSON_STRING)
			{
				// Check to see if the payload is a routable message
				PLOG(logERROR) << "Encoding is Json";
			...
			{
				PLOG(logERROR) << "Encoding is ByteArray";
				try
				{
					...
					if (bytesFull.size() > 8)
					{
						...
						PLOG(logERROR) << "Looking for abbreviated message in bytes " << bytesFull;
						uint16_t msgType;
						uint8_t msgVersion;
						uint16_t id;
						uint16_t dataLength;
						...
						if (dataLength > 0)
						{
							PLOG(logERROR) << "Checking for message type";
							switch (msgType)
							{
							case ABBR_BSM:
								PLOG(logERROR) << "Found BSM Message";
								if (bytes.size() >= 32 && dataLength >= 24)
								{
									if (!simBSM && !simLoc)
										return;

									PLOG(logERROR) << "BSM Message made";
									// extract data
									// vehicleId(4), heading*M(4), speed*K(4), (latitude+180)*M(4), (longitude+180)*M(4), elevation (4)
									BsmMessage *bsm = DecodeBsmNew(ntohl(*((uint32_t *)&(bytes.data()[8]))),
									...
									PLOG(logERROR) << "Encode bsm data";
									frame_msg = bsm->get_j2735_data();
									encodedBsm.set_data(TmxJ2735EncodedMessage<BasicSafetyMessage>::encode_j2735_message<codec::uper<MessageFrameMessage>>(frame_msg));
									// sendMsg = encode(encodedBsm, bsm);

									sendMsg = (routeable_message *)&encodedBsm;
							...
							case ABBR_SRM:
								if (bytes.size() >= 32 && dataLength >= 24)
								{
									if (!simSRM)
										return;
									PLOG(logERROR) << "Found SRM message";
									// extract data
							...
							default:
							{
								PLOG(logERROR) << "Unknown byte format.  Dropping message";
							}
								return;
							}
						}
		...
		PLOG(logERROR) << "Going forward with message.";
		...
		if (fwd)
		{
			PLOG(logERROR) << "Routing " << name << " message.";
	...
	int MessageReceiverPlugin::Main()
	{
		...
		while (_plugin->state != IvpPluginState_error)
		{
			...
			if (cfgChanged)
			{
				...
				if (port > 0 && (!server || (server->GetAddress() != ip || server->GetPort() != port)))
				{
			...
			try
			{
				...
				if (len > 0)
				{
					...
					if (verState == 1)
					{
						...
						{
							PLOG(logERROR) << " Unable to verify the incoming message: Message Verification Error and dropped \n";
